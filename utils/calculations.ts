...
export function buildCogsFreightFromSKUAndLogistics(
  ...
  const nodes = logistics.nodes.map((logNode) => {
    ...
    return {
      ...
      tollProcessing: wTollProcessing / norm,
...
export function computeChannelOutputs(
  ...
  if (logistics && skuLibrary) {
    // Compute weighted avg units per pallet from SKU library
    let totalMix = 0;
    let weightedUPP = 0;
    for (const sku of skuLibrary.skus) {
      const upp = sku.unitsPerCase * sku.casesPerPallet;
      weightedUPP += sku.volumeMixPct * upp;
      totalMix += sku.volumeMixPct;
    }
...
export function computeBlendedFinancials(
  ...
  const grossRevenue = impliedUnits * blendedGrossRevPerUnit;  // Brand's P&L top line (NOT retail sales value)
