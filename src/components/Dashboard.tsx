...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  // Per-channel EBITDA per unit — uses actual company OH per unit from Company Overhead tab
  const overheadPerUnit = companyOHPerUnit;
  ...
  return (
    <div className="space-y-6">
        ...
        <table className="table table-sm w-full">
          ...
          <tbody>
            ...
            <tr className="bg-base-300 font-bold">
              ...
              <td className="text-right font-mono text-primary" title={companyOHPerUnit > 0 ? `Plant: ${fmtCurrency(blended.plantCogs)} + OH: ${fmtCurrency(blended.effectiveOHPerUnit)}` : undefined}>{fmtCurrency(blended.blendedCogs)}</td>
      ...
      {/* Row 1: Stacked COGS + Run-Rate */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card bg-base-200">
          <div className="card-body p-4">
            <SectionHeader title="STACKED UNIT ECONOMICS" subtitle="BLENDED" />
            <OutputRow label="Plant COGS / Unit" value={fmtCurrency(blended.plantCogs)} />
            {blended.effectiveOHPerUnit > 0 && (
              <OutputRow label={`+ Company OH / Unit`} value={fmtCurrency(blended.effectiveOHPerUnit)} />
            )}
            <OutputRow label="Fully Loaded COGS / Unit" value={fmtCurrency(blended.blendedCogs)} bold />
            <div className="divider my-1"></div>
            <OutputRow label="Total Fixed Overhead (Annual)" value={fmtCurrency(blended.totalFixedCosts)} />
            {companyOHPerUnit > 0 && (
              <OutputRow label="OH / Unit at Target Volume" value={fmtCurrency(companyOHPerUnit)} />
            )}
