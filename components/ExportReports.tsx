    ...
    <>
      ...
      <table style={tbl}>
        ...
        <tbody>
          {b.channels.map(ch => (
            <tr key={ch.id}>
              <td style={td}>{CHANNEL_LABELS[ch.id] || ch.name}</td>
  ...
  return (
    <>
      ...
      <table style={tbl}>
        ...
        <tbody>
          {CHANNEL_IDS.filter(id => safe(mix[id]) > 0).map(id => {
            const co = channelOutputs[id];
            if (!co) return null;
            return (
              <tr key={id}>
                <td style={td}>{CHANNEL_LABELS[id]}</td>
      ...
      <table style={tbl}>
        ...
        <tbody>
          {ranked.map((r, i) => (
            <tr key={r.id}>
              <td style={td}>#{i + 1}</td>
              <td style={td}>{CHANNEL_LABELS[r.id]}</td>
    ...
    <>
        ...
        <tbody>
          {skuLibrary.skus.map(sku => (
            <tr key={sku.id}>
              ...
              <td style={tdR}>{fmtCurrency(safe(sku.tollProcessing))}</td>
              ...
              <td style={tdR}>{fmtPct(safe(sku.volumeMixPct))}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <table style={tbl}>
        <tbody>
          ...
          {CHANNEL_IDS.map(id => (
            <KVRow key={id} label={`${CHANNEL_LABELS[id]} Mix`} value={fmtPct(safe(mix[id]))} />
          ))}
        </tbody>
      </table>
      ...
      {CHANNEL_IDS.filter(id => safe(mix[id]) > 0).map(id => {
        ...
        return (
          <div key={id} style={{ marginBottom: 20, pageBreakInside: 'avoid' }}>
            <div style={{ fontWeight: 700, fontSize: 12, marginBottom: 6, color: '#111' }}>{CHANNEL_LABELS[id]} ({id})</div>
            <table style={tbl}>
